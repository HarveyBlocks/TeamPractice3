...
public class AdminUserController {
    ...
    public Result<List<UserInfoDto>> queryUserInfoByPage(
            ...
            @ApiParam(value = "页号,从1开始", defaultValue = "1", required = true) Integer limit,
            @PathVariable(value = "page", required = true)
            @ApiParam(value = "页长", defaultValue = ServerConstants.DEFAULT_PAGE_SIZE, required = true) Integer page) {
    ...
    public Result<List<UserInfoDto>> queryUserInfo(@RequestBody UserInfoQuery userInfoQuery) {
        return new Result<>(userService.queryUserInfo(
                userInfoQuery,
    ...
    public Result<List<PointsHistoryDto>> queryPointsHistory(
            @PathVariable(value = "user-id", required = true) @ApiParam(value = "目标用户的id", required = true)
            Long userId,
            @PathVariable(value = "limit", required = true)
